    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
				...
				local toTile = self.board.tiles[y][x]
				self:switch(self.highlightedTile, toTile, false)
...
function PlayState:switch(fromTile, toTile, swap)
    ...
    self.board.tiles[fromTile.gridY][fromTile.gridX] =  fromTile

    self.board.tiles[toTile.gridY][toTile.gridX] = toTile

    -- tween coordinates between the two so they swap
    Timer.tween(0.1, {
        [fromTile] = {x = toTile.x, y = toTile.y},
        [toTile] = {x = fromTile.x, y = fromTile.y}
    })
    
    -- once the swap is finished, we can tween falling blocks as needed
    :finish(function()
        local matches = self:calculateMatches()
		if matches == nil and swap == false then
			self:switch(toTile, fromTile, true)
		end 
    end)
end
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        Timer.tween(0.25, tilesToFall):finish(function()
			...
			if self.board:matchesAvailable() == false then
